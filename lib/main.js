...
module.exports = {
    ...
    config: {
        syncInterval: {
            title: "Sync Interval",
            description:
                "Time in seconds to check for remote changes. Changes to local settings will be detected automatically, regardless of the Sync Interval.",
            type: "number",
...
function handleWelcomeReady() {
    ...
    if (store.exists(keys.hasBeenWelcomed)) {
        inkdrop.commands.dispatch(document.body, welcomeCmds.showAuth);
    } else {
...
async function syncSettings() {
    ...
    try {
            ...
            if (searchResult == null) {
                ...
                const createResult = await gistClient
                    .create(token, {
                        files: {
                            settings: {
                                content: await getLocalSettings().then(
                                    (settings) =>
        ...
        if (syncResult instanceof Error)
            return Promise.reject(syncResult.message);
